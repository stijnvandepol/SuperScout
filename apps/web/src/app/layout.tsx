...
const TITLE =
  "SuperScout — alle supermarktaanbiedingen van deze week op één plek";

/**
 * Names the chains that are actually live, not the ten we support in principle.
 *
 * This description is inherited by every page on the site, so a hardcoded list
 * turned four broken adapters into a site-wide false claim. Reading it from the
 * data means the sentence shrinks when a chain drops out instead of lying.
 */
function description(): string {
  return `Vergelijk de aanbiedingen van deze week van ${chainSentence()} in één zoekopdracht. Dagelijks ververst, zonder account en zonder tracking.`;
}
...
export function generateMetadata(): Metadata {
  ...
  return {
    ...
    alternates: {
      types: {
        "application/rss+xml": [
          { url: "/feed.xml", title: "SuperScout — alle aanbiedingen" },
        ],
      },
    },
    openGraph: {
